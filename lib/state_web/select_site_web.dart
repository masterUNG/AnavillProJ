...
import 'package:sharetraveyard/state_web/main_home_web.dart';
...
class SelectSiteWeb extends StatefulWidget {
  const SelectSiteWeb({
    super.key,
    this.assoiate,
  });
  ...
  @override
  State<SelectSiteWeb> createState() => _SelectSiteWebState();
}

class _SelectSiteWebState extends State<SelectSiteWeb> {
  ...
  void initState() {
    ...
    AppSvervice().findCurrenAssociateLogin(docIdAssociate: widget.assoiate);
      ...
      pressFunc: () {
        if (period1model.status!) {
          ...
          if (!period1model.repair!) {
            ...
            Get.to(MainHomeWeb(
              statusRoude: period1model.statusRound ?? false,
            ));
          } else {
