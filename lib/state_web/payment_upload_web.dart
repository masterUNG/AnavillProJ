...
class _PaymentUploadWebState extends State<PaymentUploadWeb> {
  ...
  Future<void> findAssociateModel() async {
    ...
    if (kIsWeb) {
      associate = appController.currentAssociateLogin.last.associateID;
    } else {
      SharedPreferences preferences = await SharedPreferences.getInstance();
      associate = preferences.getString('user');
    }

    print('##21nov associate ----> $associate');
    ...
    await FirebaseFirestore.instance
        ...
        .get()
        .then((value) {
      print('##21nov result ---> ${value.data()}');

      asscociateModel = AsscociateModel.fromMap(value.data()!);

      print('##21nov assocateModel ---> ${asscociateModel!.toMap()}');
    });
  }
    ...
    return Scaffold(
      ...
      body: GetX(
        ...
        builder: (AppController appController) {
          print('##21nov urlImage -----> ${appController.urlImage.length}');
          return appController.urlImage.isEmpty
              ? const SizedBox()
              : Container(
